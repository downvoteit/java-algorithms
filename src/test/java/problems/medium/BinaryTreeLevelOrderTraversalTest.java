...
class BinaryTreeLevelOrderTraversalTest {
  static Stream<Arguments> source() {
    return Stream.of(
        arguments(
            new Integer[] {3, 9, 20, null, null, 15, 7}, new Integer[][] {{3}, {9, 20}, {15, 7}}),
        arguments(new Integer[] {1}, new Integer[][] {{1}}),
        arguments(new Integer[] {}, new Integer[][] {}));
  }

  private BinaryTreeLevelOrderTraversal.TreeNode arrayToTree(Integer[] input, int i) {
    if (i >= input.length || input[i] == null) return null;

    var leftNode = arrayToTree(input, i * 2 + 1);
    var rightNode = arrayToTree(input, i * 2 + 2);

    return new BinaryTreeLevelOrderTraversal().new TreeNode(input[i], leftNode, rightNode);
  }
